...
# 법정동이 아닌 건물 동 표기 (아파트 상가·근생 등). 동 추출 시 무시.
BUILDING_DONG = {'상가동', '시설동', '에이동', '비이동', '씨이동', '관리동'}

def extract_emd(addr):
    """주소에서 읍면동 추출. 읍/면 우선, 없으면 동(洞).
    아파트 주소는 '(상가동 104호 동패동)'처럼 건물 동 표기가 법정동보다
    먼저 나오므로, ①알려진 법정동을 우선 매칭하고 ②건물 동 표기는
    거른 뒤 ③괄호 뒤쪽(법정동 병기 관행)을 택한다."""
    addr = str(addr)
    m = re.search(r'파주시\s+([가-힣]+[읍면])(?![가-힣])', addr)
    if m:
        return m.group(1)
    # 앞뒤가 한글로 이어지지 않는 'XX동/XXX동' 후보를 모두 수집
    cands = re.findall(r'(?<![가-힣])([가-힣]{2,3}동)(?![가-힣])', addr)
    known = [c for c in cands if c in EMD2REGION]
    if known:
        return known[-1]          # 법정동 병기는 관행상 마지막에 옴
    real = [c for c in cands if c not in BUILDING_DONG]
    if real:
        return real[-1]
    # 도로명 힌트로 보정
